...
/* Marqueurs custom : départ (bleu), arrivée (rose) — goutte CSS classique.
   NE PAS déclarer `position` : MapLibre positionne via .maplibregl-marker
   { position: absolute } — un override fait dériver le marqueur au zoom. */
.marker-pin { width: 28px; height: 28px; }
.marker-pin::before {
